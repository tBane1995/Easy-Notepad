...
void setCursorPosition(sf::Vector2i cursor_position) {
    ...
    for (int t = 0; t < texts.size(); t++) {
        if (t == cursor_position.y) {
            ...
            if (line.size() == 0) {
                cursor.setPosition(texts[t]->getPosition());    // poprawka
                return;
            }
            ...
            sf::Vector2f endPos = texts[t]->findCharacterPos(line.size());
            ...
            if (event.type == sf::Event::Resized)
            {
                ...
                if (event.text.unicode < 128) {
                    ...
                    for (int i = 0; i < cursorPosition.y; i++) {
                        index += texts[i]->getString().toWideString().size();
                    }
                    index += cursorPosition.x;
                ...
                if(event.key.code == sf::Keyboard::Left) {
                    if (cursorPosition.x > 0) {
                        ...
                        if (cursorPosition.y > 0) {
                            cursorPosition.y -= 1;
                            cursorPosition.x = texts[cursorPosition.y]->getString().toWideString().size() - 1; //////// -1
                        }
                    }

                }
                else if (event.key.code == sf::Keyboard::Right) {
                    if (cursorPosition.x < texts[cursorPosition.y]->getString().toWideString().size()) { //////// +1
                        cursorPosition.x += 1;
                    }
                    else {
